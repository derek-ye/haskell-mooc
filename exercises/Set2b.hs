...
oddFactorial 1 = 1
oddFactorial n = 
    if n `mod` 2 == 1 then n * oddFactorial (n-1) 
...
myGcd a b
  | a < b = myGcd a (b - a)
  | a > b = myGcd (a - b) b
  | otherwise = a -- a and b are equal so doesnt matter what we return
...
leftpad s maxLen
    | maxLen > length s = leftpad (' ':s) maxLen
    | otherwise = s
...
smallestDivisorOdd :: Integer -> Integer -> Integer
smallestDivisorOdd n i
    | n `mod` i == 0 = i
    | i <= round (sqrt (fromIntegral n)) = smallestDivisorOdd n (i + 2)
    | otherwise = n

smallestDivisor :: Integer -> Integer
smallestDivisor n
    | even n = 2
    | otherwise = smallestDivisorOdd n 3
